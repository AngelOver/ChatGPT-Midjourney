...
const cn = {
  ...
  Error: {
    Unauthorized:
      "由1Chat提供支持 绘图需要/mj 前缀 \n- 点击下方发起AI绘图，`Enter`键发送 \n- 在下方直接输入你想要的图片效果，例如：`/mj 猫耳少女（建议英文，”/mj cat girl“）（中文效果不佳，需自行翻译描述词）`  \n- 对于 `Midjourney` 模型，图片生成时间一般较长，默认超时时间为 `5` 分钟，请耐心等待。。。（等完没反应就是人太多了，晚上再来）\n- 图生图模式，对话框上最后按钮可上传图片  \n-  mj绘画Q群，662783138 ,绘画模型、mj账号等  \n- 支持 U1-U4 放大图片，MJ模型暂定“NiJi动漫模型” ，其他模型联系群主 \n- 当前绘画子域 mj.c3g.ink ，1Chat主域 1Chat.vin ",
  },
  Auth: {
    Title: "需要密码",
  ...
  Store: {
    DefaultTopic: "新的绘画",
    BotHello:  "由1Chat提供支持 绘图需要/mj 前缀 \n- 点击下方发起AI绘图，`Enter`键发送 \n- 在下方直接输入你想要的图片效果，例如：`/mj 猫耳少女（建议英文，”/mj cat girl“）（中文效果不佳，需自行翻译描述词）`  \n- 对于 `Midjourney` 模型，图片生成时间一般较长，默认超时时间为 `5` 分钟，请耐心等待。。。（等完没反应就是人太多了，晚上再来）\n- 图生图模式，对话框上最后按钮可上传图片  \n-  mj绘画Q群，662783138 ,绘画模型、mj账号等  \n- 支持 U1-U4 放大图片，MJ模型暂定“NiJi动漫模型” ，其他模型联系群主 \n- 当前绘画子域 mj.c3g.ink ，1Chat主域 1Chat.vin ",
    Error: "出错了，稍后重试吧",
    Prompt: {
